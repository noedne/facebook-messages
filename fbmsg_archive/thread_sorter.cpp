...
const int min_N = 1, max_N = 6;//inclusive exclusive

dict ngram_freqs(const str_list& corpus, int n) {
    std::vector<str_list> ngrams;
    for (auto it = corpus.begin() + n; it != corpus.end(); ++it) {
        str_list ngram(it - n, it);
        if (std::find(ngram.begin(), ngram.end(), std::string(1, DELIM)) == ngram.end())
            ngrams.push_back(ngram);
    }
    dict freq_dict;
    for (str_list ngram : ngrams)
        freq_dict[ngram]++;
    return freq_dict;
}

dict combine_dicts(dict& a, const dict& b) {
    for (auto it = b.begin(); it != b.end(); ++it)
        a[it->first] += it->second;
    return a;
}

bool overlap(const str_list& a, const str_list& b) {
    int max_overlap = std::min(3, (int)std::min(a.size(), b.size()));
    return std::search(b.begin(), b.end(), a.begin(), a.begin() + max_overlap) != b.end()
        || std::search(b.begin(), b.end(), a.end() - max_overlap, a.end()) != b.end()
        || std::search(a.begin(), a.end(), b.begin(), b.begin() + max_overlap) != a.end()
        || std::search(a.begin(), a.end(), b.end() - max_overlap, b.end()) != a.end();
}

std::vector<str_list> corpus_list_from_chat(int chat) {
    std::vector<str_list> corpus_list;
    for (message me : thread_list[chat].msg_list) {
        if (me.msg.empty())
            continue;
        std::string s(me.sender);
        size_t pos = std::find(candidates.begin(), candidates.end(), s) - candidates.begin();
        if (pos == candidates.size()) {
            candidates.push_back(s);
            corpus_list.push_back({});
        }
        s.clear();
        ...
        corpus_list[pos].insert(corpus_list[pos].end(), p.begin(), p.end());
        corpus_list[pos].push_back(std::string(1, DELIM));
    }
    const std::string ALIAS[][2] {
        {"Allen Li", "Alluhn Ri"},
        {"Alex Li", "Alexander Li"},
        {"Jimmy Cheng", "Jimmah Chan"},
        {"Navid Mokhlesi", "Navidad Mokhlease"}
    };
    for (std::string alias[2] : ALIAS) {
        auto it1 = std::find(candidates.begin(), candidates.end(), alias[0]);
        auto it2 = std::find(candidates.begin(), candidates.end(), alias[1]);
        if (it1 != candidates.end() && it2 != candidates.end()) {
            int i1 = it1 - candidates.begin(), i2 = it2 - candidates.begin();
            corpus_list[i1].insert(corpus_list[i1].end(), corpus_list[i2].begin(), corpus_list[i2].end());
            candidates.erase(it2);
            corpus_list.erase(corpus_list.begin() + i2);
        }
    }
    for (size_t i = candidates.size(); i --> 0 ;)
    if (corpus_list[i].size() < 100) {
        candidates.erase(candidates.begin() + i);
        corpus_list.erase(corpus_list.begin() + i);
    }
    if (corpus_list.size() < 2)
        corpus_list.clear();
    return corpus_list;
}

std::vector<dict> freq_dicts_from_corpus_list(const std::vector<str_list>& corpus_list) {
    std::vector<dict> freq_dicts(candidates.size());
    for (int n = min_N; n < max_N; n++)
    for (size_t candidate = 0; candidate < candidates.size(); candidate++) {
        str_list corpus = corpus_list[candidate];
        dict dict_to_add = ngram_freqs(corpus, n);
        freq_dicts[candidate] = combine_dicts(freq_dicts[candidate], dict_to_add);
    }
    return freq_dicts;
}

std::vector<dictd> tfidf_dicts_from_freq_dicts(std::vector<dict>& freq_dicts) {
    std::vector<dictd> tfidf_dicts(candidates.size());
    dict num_containing;
    for (size_t candidate = 0; candidate < candidates.size(); candidate++)
        for (auto& p : freq_dicts[candidate])
            num_containing[p.first]++;
    for (size_t candidate = 0; candidate < candidates.size(); candidate++)
    for (auto& p : freq_dicts[candidate]) {
        str_list ngram = p.first;
        int tf = freq_dicts[candidate][ngram];
        double idf = log((double)candidates.size() / num_containing[ngram]);
        tfidf_dicts[candidate][ngram] = tf * idf * ngram.size();
        const std::string KILL[]{"and", "or", "of", "with"};
        for (std::string kill : KILL)
        if (ngram.back() == kill) {
...
std::vector<dictd> prune_substrings(std::vector<dictd>& tfidf_dicts, size_t prune_thru = 1000) {
    for (size_t candidate = 0; candidate < candidates.size(); candidate++) {
        std::vector<str_list> so_far;
        std::vector<std::pair<double, str_list>> ngrams_sorted;
        for (auto& p : tfidf_dicts[candidate])
            ngrams_sorted.push_back(std::make_pair(p.second, p.first));
        std::sort(ngrams_sorted.begin(), ngrams_sorted.end(), std::greater<std::pair<double, str_list>>());
        if (ngrams_sorted.size() > prune_thru)
            ngrams_sorted.resize(prune_thru);
        for (auto& ngram : ngrams_sorted) {
            bool best = 1;
            for (auto& better_ngram : so_far)
                if (overlap(better_ngram, ngram.second)) {
                    tfidf_dicts[candidate][ngram.second] = 0;
                    best = 0;
                    break;
            }
            if (best)
                so_far.push_back(ngram.second);
        }
    }
    return tfidf_dicts;
}

std::vector<std::pair<double, str_list>> top_ngrams_for_candidate(const std::vector<dictd>& tfidf_dicts, int candidate, size_t total = 20) {
    std::vector<std::pair<double, str_list>> top_ngrams;
    for (auto& p : tfidf_dicts[candidate])
        top_ngrams.push_back(std::make_pair(p.second, p.first));
    std::sort(top_ngrams.begin(), top_ngrams.end(), std::greater<std::pair<double, str_list>>());
    if (top_ngrams.size() > total)
        top_ngrams.resize(total);
    return top_ngrams;
}

void calc_repeated_phrases_old() {
    std::ofstream ofs(AFNAME);
    for (size_t i = 0; i < thread_list.size(); i++) {
        ofs << thread_list[i].name << " (" << thread_list[i].msg_list.size() << ")\n";
        candidates.clear();
        auto corpus_list = corpus_list_from_chat(i);
        if (corpus_list.empty()) continue;
        auto freq_dicts = freq_dicts_from_corpus_list(corpus_list);
        auto tfidf_dicts = tfidf_dicts_from_freq_dicts(freq_dicts);
        tfidf_dicts = prune_substrings(tfidf_dicts);
        for (size_t candidate = 0; candidate < candidates.size(); candidate++) {
            ofs << candidates[candidate] << " (" << corpus_list[candidate].size() << ")\n";
            auto top_ngrams = top_ngrams_for_candidate(tfidf_dicts, candidate, 20);
            for (auto& p : top_ngrams) {
